...
public class AuxThread extends Thread {
    ...
    public void run(NewsRepository newsRepository, int index, int pageSize, String sortString, boolean ascending) {

        this.result = ascending
                ? newsRepository.findAll(PageRequest.of(index, pageSize, Sort.by(sortString).ascending()))
                : newsRepository.findAll(PageRequest.of(index, pageSize, Sort.by(sortString).descending()));
    }
