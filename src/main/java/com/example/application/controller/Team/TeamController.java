...
public class TeamController {
  ...
  createTeamWithManager(LoginUser loginUser, List<Integer> jogadores,
                        String escalaoI, String name) {
    ...
    Team createdTeam = teamService
                           .criarEquipa(teamRepository, loginUser.getId(),
                                        jogadores, escalao, name)
                           .success;
  ...
  public ResponseEntity<ResponseType<Team>>
  createTeamWithAdmin(LoginUser loginUser, List<Integer> equipa,
                      String escalaoI, String name, Integer managerId) {
    Escalao escalao = Escalao.valueOf(escalaoI.toUpperCase());
    ...
    Team createdTeam =
        teamService
            .criarEquipa(teamRepository, managerId, equipa, escalao, name)
            .success;
  ...
  public ResponseEntity<ResponseType<Team>>
  editTeam(LoginUser currentUser, Integer teamId, Integer managerId,
    ...
    Team team = null;
    try {
      team = teamRepository.findById(teamId.longValue());
    } catch (Exception e) {
      var response = new ResponseType<Team>();
    ...
    if (existingTeam != null &&
        !existingTeam.getId().equals(teamId.longValue())) {
      var response = new ResponseType<Team>();
      response.error("O nome da equipa já existe");
      return ResponseEntity.badRequest().body(response);
    }
    ...
    for (Integer user : jogadoresNovos) {
      if (jogadoresEmEquipas.contains(user)) {
        ...
        var response = new ResponseType<Team>();
        response.error(atleta.getFirstname() + " " + atleta.getLastname() +
                       " já pertence a outra equipa");
        return ResponseEntity.badRequest().body(response);
      }
    }
    ...
    Team editedTeam =
        teamService
            .editarEquipa(teamRepository, teamId, managerId, equipa, name)
            .success;
  ...
  public ResponseEntity<ResponseType<Boolean>> removeTeam(LoginUser loginUser,
    ...
    if ((!user.getId().equals(team.getManagerID()) ||
    ...
    Team deletedTeam =
        teamService.removerEquipa(teamRepository, teamId).success;
  ...
  public ResponseEntity<ResponseType<Team>>
  addPlayer(LoginUser currentUser, Integer teamId, List<Integer> equipa) {
    // verificar se o token é válido
    var isValidToken = TokenService.validateToken(
        currentUser, currentUser.getStringToken(), service);
    if (!isValidToken) {
    ...
    Team addedPlayerTeam =
        teamService.adicionarJogador(teamRepository, teamId, equipa).success;
  ...
  public ResponseEntity<ResponseType<Team>>
  removePlayer(LoginUser loginUser, Integer teamId,
               List<Integer> jogadoresRemovidos) {

    User user = usersRepository.findById(loginUser.getId()).get();
    // verificar se o token é válido
    var isValidToken = TokenService.validateToken(
        loginUser, loginUser.getStringToken(), service);
    if (!isValidToken) {
      var response = new ResponseType<Team>();
      response.error("Token inválida");
      return ResponseEntity.badRequest().body(response);
    ...
    if (teamRepository.findById(teamId.longValue())
            .getPlayers()
            .contains(jogadoresRemovidos)) {
      var response = new ResponseType<Team>();
      response.error("Os atletas nao pertecem a esta equipa");
      return ResponseEntity.badRequest().body(response);
    }

    Team removedPlayerTeam =
        teamService.removerJogador(teamRepository, teamId, jogadoresRemovidos)
            .success;
  ...
  public ResponseEntity<ResponseType<Team>>
  switchManager(LoginUser currentUser, Integer teamId, Integer managerId) {
    ...
    Team changedManagerTeam =
        teamService.trocarTreinador(teamRepository, teamId, managerId).success;
  ...
  public ResponseEntity<ResponseType<Boolean>>
  isPlayerInTeam(List<Integer> atletas) {
   ...
   */
  public Team getTeamExample() { return null; }
}
