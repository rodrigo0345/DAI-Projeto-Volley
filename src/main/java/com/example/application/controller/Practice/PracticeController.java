...
public class PracticeController {
  ...
  public ResponseEntity<ResponseType<Practice>> createPractice(Integer teamID,
    ...
    if (team == null || team.getManagerID() == null ||
        team.getPlayers() == null || team.getPlayers().isEmpty()) {
      var response = new ResponseType<Practice>();
      response.error("A equipa não existe");
      return ResponseEntity.badRequest().body(response);
    }
    boolean teamHasManager =
        teamRepository.findById(teamID.longValue()).getManagerID() != null;
    if (!teamHasManager) {
      var response = new ResponseType<Practice>();
    ...
    Practice newPractice =
        PracticeService
            .createPractice(practiceRepository, teamID.longValue(), local,
  ...
  @AnonymousAllowed
  public ResponseEntity<ResponseType<Practice>>
  editPractice(LoginUser loginUser, String local, String startDate,
               String endDate, Long practiceID) {
    ...
    if (!(user.getId().equals(
            teamRepository.findById(practice.getTeam()).getManagerID()))) {
      var response = new ResponseType<Practice>();
    ...
    Practice editedCall = PracticeService
                              .editPratice(practiceRepository, practice, local,
                                           startDateTime, endDateTime)
                              .success;
  ...
  public ResponseEntity<ResponseType<Practice>>
  removePractice(Long practiceID, LoginUser loginUser) {
    ...
    Practice deletedCall =
        PracticeService.removePratice(practiceRepository, practice).success;
  ...
  public boolean doesLocalOverlap(Practice other, LocalDateTime startDate,
                                  LocalDateTime endDate) {
    return !startDate.isAfter(other.getEndDate()) &&
        !endDate.isBefore(other.getStartDate()) &&
        (!startDate.isEqual(other.getStartDate()) ||
         !endDate.isEqual(other.getEndDate()));
  }
